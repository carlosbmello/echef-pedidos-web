...
// criarPedidoAPI, salvarPedidoParaSincronizacaoDB, uuidv4, PedidoOfflinePayload, BackendPedidoPayload Removidos
import {
...
// FiSend, FiMapPin removidos. TipoItemPedido removido da import de comanda.
import { FiArrowLeft, FiPlus, FiMinus, FiTrash2, FiEdit2, FiShoppingBag, FiChevronLeft, FiMessageSquare, FiSearch, FiClipboard } from 'react-icons/fi';
...
const PedidoPage: React.FC = () => {
  ...
  const [idNumericoDaComanda, setIdNumericoDaComanda] = useState<number | null>(null); // Renomeado de idNumericoComandaOriginal
  const [localInformadoNaBusca, setLocalInformadoNaBusca] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const localPImpressao = location.state?.localInformadoParaImpressao as string | null | undefined;
    const itensRetornando = location.state?.itensPedidoRetornandoDaRevisao as ItemPedidoState[] | undefined;
    const obsGeralRetornando = location.state?.observacaoGeralRetornandoDaRevisao as string | undefined;
    const idNumericoUrl = comandaIdFromUrl ? parseInt(comandaIdFromUrl, 10) : null;

    const popularDadosComanda = (dados: ComandaDetalhada) => { // Removido 'origem'
      setNumeroComandaExibicao(dados.numero);
      setNomeClienteComanda(dados.cliente_nome || null);
      setObservacaoGeralInput(obsGeralRetornando !== undefined ? obsGeralRetornando : (dados.observacao_geral || ''));
      setObservacaoGeralOriginalDaComanda(dados.observacao_geral || ''); // Guarda a original
      setComandaStatusAtual(dados.status);
      setTotalJaConsumidoState(Number(dados.total_atual_calculado) || 0);
      setIdNumericoDaComanda(dados.id);
      setLocalInformadoNaBusca(localPImpressao || dados.local_atual || null);
      setIsLoadingComanda(false);
    };

    if (itensRetornando) {
        setItensPedido(itensRetornando);
        if (dadosComandaDoLocation && idNumericoUrl && dadosComandaDoLocation.id === idNumericoUrl) {
            popularDadosComanda(dadosComandaDoLocation);
             ...
             buscarComandaDetalhadaPorIdAPI(idNumericoUrl).then(cd => {
                 if(cd) popularDadosComanda(cd); else {toast.error("Erro dados comanda."); navigate('/comandas');}
             }).catch(()=> {toast.error("Erro buscar comanda."); navigate('/comandas');}).finally(()=> setIsLoadingComanda(false));
        }
      ...
      buscarComandaDetalhadaPorIdAPI(idNumericoUrl)
        ...
        .catch(() => { toast.error("Falha dados comanda."); navigate('/comandas', {replace: true}); })
        .finally(() => setIsLoadingComanda(false));
    } else { toast.error("ID comanda inválido."); navigate('/comandas', {replace: true}); }
    carregarCardapio();
  }, [comandaIdFromUrl, location, navigate, carregarCardapio]);
  ...
  const handleProsseguirParaRevisao = () => {
    ...
    const dadosComandaOriginalParaRevisao = {
        ...
        local_atual: localInformadoNaBusca, 
    ...
    navigate(
      ...
      { state: { 
          ...
          localInformadoParaImpressao: localInformadoNaBusca 
        } 
      }
    );
  };
  ...
  if (!cardapio) { return ( <div className="p-4 text-center"> <p className="text-red-500">Falha ao carregar cardápio. {statusConexao === 'offline' && "Você está offline."}</p> <button onClick={carregarCardapio} className="mt-2 px-3 py-1 bg-blue-500 text-white rounded">Tentar Novamente</button> </div> ); }
  if (!idNumericoDaComanda && !isLoadingComanda) { return <div className="p-4 text-center">Dados da comanda não carregados.</div>;}
    ...
    <div className="flex flex-col md:flex-row h-[calc(100vh-4rem)]">
      <div className="w-full md:w-3/5 p-4 border-r border-gray-200 flex flex-col overflow-hidden">
        ...
        <div className="mb-4 flex-shrink-0">
          {mostrarApenasCategorias && cardapio.categorias && ( <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2"> {cardapio.categorias.map((cat) => ( <button key={cat.id} onClick={() => handleSelectCategoria(cat)} className="p-3 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-blue-500 hover:text-white rounded-lg shadow transition-colors duration-150 flex flex-col items-center justify-center h-24 min-h-[6rem]"> <span>{cat.nome}</span> </button> ))} </div> )}
      ...
      <div className="w-full md:w-2/5 p-4 overflow-y-auto bg-gray-50 flex flex-col">
         <div className='flex-shrink-0'>
            ...
            <div className="mb-4"><label htmlFor="observacaoGeral" className="block text-sm font-medium"><FiMessageSquare className="inline mr-1"/>Obs. Geral (Pedido Atual)</label><textarea id="observacaoGeral" value={observacaoGeralInput} onChange={e => setObservacaoGeralInput(e.target.value)} rows={2} className="w-full p-2 border rounded text-sm" placeholder="Ex: Alergias..." /></div>
        </div>
        ...
        <div className="flex-shrink-0 pt-4">
            {(itensPedido.length > 0 || observacaoGeralInput.trim() !== observacaoGeralOriginalDaComanda ) && ( 
              <button onClick={handleProsseguirParaRevisao} disabled={comandaStatusAtual?.toLowerCase() !== 'aberta'} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-md shadow-sm flex items-center justify-center disabled:opacity-50"> 
                <FiClipboard className="mr-2"/> Revisar Pedido ({itensPedido.length} novo(s))
              </button> 
            )}
            {comandaStatusAtual?.toLowerCase() !== 'aberta' && (itensPedido.length > 0 || observacaoGeralInput.trim() !== observacaoGeralOriginalDaComanda) && <p className="text-xs text-red-500 text-center mt-2">Comanda não está aberta.</p>}
        </div>
      </div>
    </div>
  );
